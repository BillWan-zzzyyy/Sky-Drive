...
本文件用于在 CARLA 中从BEV视角 RGB 相机采集一张鸟瞰图。
1. Start CARLA
...
CONFIG = {
    ...
    "output": {
        ...
        "filename_pattern": "town10hd_birdseye_{ts}.png",  # {ts} will be replaced by timestamp
        "overwrite_single": False,  # True -> fixed name; False -> timestamped
    },
    "debug": {
...
def build_output_path(dir_path: str, pattern: str, overwrite_single: bool) -> str:
    ...
    Args:
        ...
        pattern (str): Pattern with optional {ts}
        overwrite_single (bool): Whether to overwrite a single fixed name
...
def load_town(client: carla.Client, town_map: str) -> carla.World:
    ...
    Returns:
        carla.World: World of the loaded map 
    """
...
def set_sync_mode(world: carla.World, enable: bool) -> dict:
    ...
    Args:
        world (carla.World): The world
        enable (bool): True to enable

    Returns:
        dict: Previous settings snapshot
    """
...
def save_image_to_disk(image: carla.Image, output_path: str) -> None:
    ...
    Args:
        image (carla.Image): Image object from sensor
        output_path (str): Absolute file path
...
def listen_and_capture_one_frame(camera: carla.Sensor, output_path: str, done_event: threading.Event) -> None:
    ...
    Args:
        camera (carla.Sensor): Spawned camera actor
        output_path (str): Save path
        done_event (threading.Event): Completion flag
    """
...
def spawn_camera(world: carla.World) -> carla.Sensor:
    ...
    Returns:
        carla.Sensor: The spawned camera actor
    """
